...
function sessionTagIds(session: ScheduleSessionViewModel): Set<TagId> {
  const ids =
    session.session.tagIds.length > 0 ? session.session.tagIds : session.tags.map((t) => t.id);
  return new Set(ids);
}
...
function sessionMatchesNormalizedTagGroups(
  ...
  const ids = sessionTagIds(session);
...
export function buildScheduleFilterPath(confSlug: string, searchParams: URLSearchParams): string {
  ...
  return `/${encodeURIComponent(confSlug)}/filters/${query ? `?${query}` : ""}`;
}
