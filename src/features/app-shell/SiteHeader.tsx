          ...
          <div className="flex min-w-0 items-center gap-2.5 sm:gap-4">
            ...
            >
              <span className="flex min-w-0 items-center gap-3">
                <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-white/10 bg-white/4 text-[0.7rem] font-semibold tracking-[0.18em] text-(--dc34-accent-secondary) shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] md:hidden">
                  {conference.code}
                </span>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <nav aria-label="Primary">
              <details className="group relative">
                <summary
                  className={`ui-btn-base ui-btn-secondary flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-xl border-white/10 bg-white/4 px-3.5 text-left text-sm text-slate-100 shadow-[0_10px_28px_rgba(2,6,23,0.18)] transition duration-200 ease-out group-open:border-(--dc34-accent-primary)/36 group-open:bg-(--dc34-bg-primary)/50 group-open:text-white group-open:shadow-[0_18px_42px_rgba(2,6,23,0.3)] [&::-webkit-details-marker]:hidden ${focusRingClass}`}
                >
                  <RocketLaunchIcon
                  ...
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-white/4.5 text-slate-400 transition-colors group-open:bg-(--dc34-accent-primary)/18 group-open:text-(--dc34-accent-secondary)">
                    <ChevronDownIcon
                ...
                <div className="absolute top-full right-0 w-[min(24rem,calc(100vw-1rem))] pt-2">
                  ...
                  >
                    <span className="h-2.5 w-px rounded-full bg-(--dc34-accent-primary)/35" />
                  </div>
                    ...
                    <ul className="grid gap-1.5">
                      {menuItems.map(({ title, href, description, icon: Icon }) => {
                        ...
                        const itemClassName = `group/item relative flex items-start gap-3 overflow-hidden rounded-[1.2rem] border px-3 py-3 text-left transition duration-200 ease-out ${focusRingClass} ${
                          isActive
                            ? "border-(--dc34-accent-primary)/36 bg-[linear-gradient(135deg,rgba(13,41,74,0.86),rgba(15,23,42,0.96))] text-white shadow-[0_14px_34px_rgba(2,6,23,0.24),inset_0_1px_0_rgba(255,255,255,0.06)]"
                        ...
                        const iconClassName = `mt-0.5 flex h-9 w-9 shrink-0 items-center justify-center rounded-[1rem] border shadow-[inset_0_1px_0_rgba(255,255,255,0.08)] transition-colors ${
                          isActive
                            ? "border-(--dc34-accent-primary)/30 bg-(--dc34-accent-primary)/12 text-(--dc34-accent-secondary)"
                            : "border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.06),rgba(255,255,255,0.02))] text-slate-400 group-hover/item:border-(--dc34-accent-primary)/24 group-hover/item:bg-(--dc34-accent-primary)/10 group-hover/item:text-(--dc34-accent-secondary) group-focus-within/item:border-(--dc34-accent-primary)/24 group-focus-within/item:bg-(--dc34-accent-primary)/10 group-focus-within/item:text-(--dc34-accent-secondary)"
                        }`;
                        const trailingClassName = `mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full border transition-all duration-200 ${
                          isActive
                            ? "border-(--dc34-accent-primary)/26 bg-(--dc34-accent-primary)/14 text-(--dc34-accent-secondary)"
            ...
            <Link
              ...
              className={`ui-icon-btn h-11 w-11 rounded-xl border-white/10 bg-white/3 text-slate-300 shadow-[0_10px_28px_rgba(2,6,23,0.18)] transition ${focusRingClass} ${
                isAppsPage
                  ? "border-(--dc34-accent-primary)/35 bg-(--dc34-accent-primary)/12 text-(--dc34-accent-secondary)"
                  : "hover:border-(--dc34-accent-primary)/28 hover:bg-(--dc34-accent-primary)/8 hover:text-(--dc34-accent-secondary)"
              }`}
